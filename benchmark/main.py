...
def main(args):
    ...
    if args.finetune_path is not None:
        model, _, preprocess = get_finetuned_model(
            device, args.pretrained, args.finetune_args
        )
        tokenizer = get_tokenizer(
            args.finetune_args.model, context_length=args.finetune_args.context_length
        )
    else:
    ...
    if args.reranker_finetune_path is not None:
        reranker_model, _, _ = get_finetuned_model(
            device, args.pretrained_reranker, args.reranker_args
        )
    ...
    if os.path.isdir(save_path):
        raise FileExistsError(
            "Benchmark directory already exist. Don't overwrite your data :("
        )
    ...
    (
        ...
        reg_retrieval=args.reg_retrieval,
        mask_padding=args.mask_padding,
    )
    ...
    if reranker_model is not None:
        (
            ...
            batch_size=args.batchsize,
            reg_retrieval=False,
            second_to_last=args.second_to_last,
        )
