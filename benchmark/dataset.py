...
def encode_dataset(
    ...
    for images, (texts_and_masks, original_texts) in tqdm(dataloader):
        ...
        all_captions += list(chain.from_iterable(list(map(list, zip(*original_texts)))))
        ...
        if reg_retrieval:
            ...
            _, image_embeddings = clip.encode_image(
                images, return_tokens=True, second_to_last=second_to_last
            )
            _, text_embeddings = clip.encode_text(
                text, return_tokens=True, second_to_last=second_to_last
            )
...
def get_dataset(args, transform, tokenizer, mask_padding, repeat_tokens):
    dataset = CocoCaptions(
        ...
        target_transform=lambda texts: (
            tokenizer(texts[:5], return_mask=mask_padding, repeat_tokens=repeat_tokens),
            texts[:5],
        ),
    )
    return dataset
