...
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, ColbertLoss, ColbertDistillClipLoss
...
from finetune.configure_finetune import configure_model
...
def create_loss(args):
    if args.distill and args.colbert:
        return ColbertDistillClipLoss(
            ...
            include_contrastive=args.distill_include_contrastive
        ) 
    elif args.distill:
        ...
        return SparcLoss(
                ...
                global_lambda=args.sparc_global_lambda
        )
    elif args.colbert:
        return ColbertLoss(
            local_loss=args.local_loss,
...
def create_model_and_transforms(
    ...
    # New gateway to add modules to CLIP Model
    if finetune_args is not None:
        configure_model(model, finetune_args)

    # Load weights of module from pretrained
    if finetune_path is not None:
        state_dict = load_state_dict(finetune_path)
